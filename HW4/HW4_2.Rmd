...
clust1_sum
```
we can see that cluster 9 has the largest size. To do analysis, we select top 3 clusters (cluster 9, 2, 6), we select the top three largest clusters to analyze their characteristics, so as to maximize the positioning of their potential consumers.

```{r biggest cluster, echo=FALSE}
biggest_clust <-  clust1_sum[clust1_sum$cluster_No == 9 | clust1_sum$cluster_No == 2 | clust1_sum$cluster_No == 6, ]
target <- c(9,2,6)
...
we can see that cluster 2 has the largest size. So we focus on this biggest cluster. Finally, after analyzing we still use the conclusion from __kmeans__ and __kmeans++__. 
